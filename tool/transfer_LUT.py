...
def DT2HDL(file_read_path, module_name, input_columns, input_bits=8):
    ...
    with open(file_write_path, 'w') as file:  # start to write the file
        ...
        for i in range(nodes.shape[0]):
            nodes_numbers += (
                        'ns' + str(nodes_number[i]) + ',')  # gets the serial numbers of every node, same as it in .dot
        ...
        for i in range(leaves.shape[0]):
            ...
            while not end:
                chain = chains.index[
                    chains['next'] == leaf].tolist()  # get the number of the chain with the behind the leaf
            ...
            for j in range(len(nodes_number)):  # the length of it is same as the number of nodes
                x = conditions.index[
                    conditions['number'] == nodes_number[j]].tolist()  # check the condition for every node
